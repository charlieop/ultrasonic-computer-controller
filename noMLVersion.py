...
def detectMovementStart(mag_data, targetIndex, radiusOfInterest, rangeOfInterest):
    ...
    if  np.average(rangeOfInterest[0: radiusOfInterest - 4]) > movementThreshold:
        left = True
        return True
    if np.average(rangeOfInterest[radiusOfInterest + 5: 2 * radiusOfInterest + 1] > movementThreshold):
        right = True
        return True
...
while True:
    ...
    if(time.time() - lastMovementRecordTime > 1):
        if inDetection:
            print(f"Finish detection: Left-{left}, Right-{right}")
            if left and right:
                print("tap")
            elif left:
                print("up")
        ...
        detectMovementStart(mag_data, targetIndex, radiusOfInterest, rangeOfInterest)
